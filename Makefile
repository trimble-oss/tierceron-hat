...
brim: */*.go
	$(GO_VARS) GOFIPS140=certified $(GO) build -tags "fips" -o="$(ROOT)/bin/brim" -ldflags="$(LD_FLAGS)" $(ROOT)/brim/brim.go
...
captip: */*.go
	$(GO_VARS) GOFIPS140=certified $(GO) build -tags "fips" -o="$(ROOT)/bin/tipcap" -ldflags="$(LD_FLAGS)" $(ROOT)/captip/captip.go
	chmod 700 $(ROOT)/bin/tipcap

captiptwo: */*.go
	$(GO_VARS) GOFIPS140=certified $(GO) build -tags "fips" -o="$(ROOT)/bin/tipcaptwo" -ldflags="$(LD_FLAGS)" $(ROOT)/captiptwo/captip.go
	chmod 700 $(ROOT)/bin/tipcaptwo

captap: */*.go
	$(GO_VARS) GOFIPS140=certified $(GO) build -tags "fips" -o="$(ROOT)/bin/captap" -ldflags="$(LD_FLAGS)" $(ROOT)/captap/captap.go
	chmod 700 $(ROOT)/bin/captap

crown: */*.go
	$(GO_VARS) GOFIPS140=certified $(GO) build -tags "fips" -o="$(ROOT)/bin/crown" -ldflags="$(LD_FLAGS)" $(ROOT)/crown/main.go
	chmod 700 $(ROOT)/bin/crown

tiara: */*.go
	$(GO_VARS) GOFIPS140=certified $(GO) build -tags "fips" -o="$(ROOT)/bin/tiara" -ldflags="$(LD_FLAGS)" $(ROOT)/tiara/main.go
